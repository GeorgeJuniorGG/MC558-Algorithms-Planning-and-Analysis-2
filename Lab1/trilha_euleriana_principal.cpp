...
int main()
{
    ...
    bool achou; // Variavel que indica se a rotina encontrou ou nao uma trilha euleriana.

    achou = trilha_euleriana(grafo.V, grafo.M, grafo, origem, destino, trilha, &mensagem, &RA);
    ...
    if (!achou)
    {
        // Caso a rotina nao tenha encontrado uma trilha euleriana, imprime o motivo, que está na variavel mensagem.
        cout << mensagem << endl;
    }
    ...
    else if (verificarTrilha(grafo.V, grafo.M, origem, destino, grafo))
    {
        ...
        {
            cout << trilha[i] << " " << origem[i] << " " << destino[i] << endl;
        }
        cout << trilha[grafo.M] << " FIM" << endl;
    }
    return 0;
}